...
module Puppet
  Type.newtype(:wls_server_2) do
    ...
    to_get_raw_resources do
      ...
      environment = { 'action' => 'index', 'type' => 'wls_server_2' }
      wlst template('puppet:///modules/orawls/providers/wls_server_2/index.py.erb', binding), environment
    end

    on_create do | command_builder |
      wlst_action = 'create'
      Puppet.info "create #{name} "
      template('puppet:///modules/orawls/providers/wls_server_2/create.py.erb', binding)
    end

    on_modify do | command_builder |
      wlst_action = 'modify'
      Puppet.info "modify #{name} "
      template('puppet:///modules/orawls/providers/wls_server_2/modify.py.erb', binding)
    end

    on_destroy do | command_builder |
      Puppet.info "destroy #{name} "
      template('puppet:///modules/orawls/providers/wls_server_2/destroy.py.erb', binding)
    end
